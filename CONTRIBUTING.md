# Contributing to tessera-crewai
...
Open an issue at [github.com/tessera-llm/tessera-crewai/issues](https://github.com/tessera-llm/tessera-crewai/issues) with:

- Package version (`pip show tessera-crewai`)
- CrewAI version
...
This package is a thin adapter on top of the CrewAI framework. The Tessera wrapper exposes a one-line config helper that routes CrewAI's LLM calls through the Tessera proxy at `api.tesseraai.io`. Keep new helpers tiny -- one config-factory per upstream provider, no business logic inside the wrapper.
